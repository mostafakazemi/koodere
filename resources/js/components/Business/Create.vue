<template>
    <div class="container">
        <div class="card my-5">
            <h5 class="card-header">ثبت شغل</h5>
            <div class="card-body">
                <div class="alert alert-primary" role="alert" v-if="!user.type">
                    برای ثبت شغل ابتدا
                ...
                <div class="alert alert-warning" role="alert" v-else-if="user.type==='business'">
                    <h4 class="alert-heading">هشدار!</h4>
                    <p>شما قبلا شغلتان را ثبت کرده‌اید!</p>
                    <hr>
                    <p class="mb-0">
                        <router-link to="/business/panel" class="btn btn-lg btn-warning">ورود به صفحه پنل شغلی
                        </router-link>
                    </p>
                </div>
                    ...
                    <form @submit.prevent="handleSubmit(onSubmit)">
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <ValidationProvider tag="div" name="name" rules="required" v-slot="{ classes, errors }">
                                    <input type="text" v-model="cmp.name" class="form-control" dir="auto"
                                           :class="classes">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <ValidationProvider tag="div" name="job" :rules="jobRules"
                                                    v-slot="{ classes, errors }">
                                    <input type="text" id="job" v-model="cmp.job" ref="job" class="form-control"
                                           :class="classes">
                                    <span class="v-invalid-feedback">{{ errors[0] }}</span>
                                </ValidationProvider>
                            </div>
                        ...
                        <div class="form-row">
                            <div class="form-group w-100 col-md-10 col-lg-8">
                                ...
                                <ValidationProvider tag="div" name="latitude" rules="required"
                                    ...
                                    <span class="v-invalid-feedback"
                                          v-if="errors[0]">آدرس دقیق روی نقشه انتخاب کنید!</span>
                                </ValidationProvider>
                                ...
                                <div id="map" class="cedarmap rounded-1 mx-3" style="height: 400px">
                                    <div class="form-group">
                                        <input type="search" id="streets" name="streets"
                                               class="form-control col-8 col-md-6" placeholder="جستجو">
                                    </div>
...
<script>
    export default {
        ...
        computed: {
            jobRules: function () {
                return 'required|oneOf:' + this.allJobs
            }
        },
        beforeCreate() {
            window.scrollTo(0, 0)
        ...
        mounted() {
            if (this.user.type === 'user') {
                ...
                $(this.$refs.job).on("autocompleteselect", (event, ui) => {
                    this.cmp.job = ui.item.value;
                });
        ...
        methods: {
            onSubmit() {
                ...
                axios.post('/add-business', this.cmp)
                    .then(response => {
                        if (response.data.message === "Business added") {
                            this.user.business = response.data.business
                            this.user.business.categories = []
                            Swal.fire({
                                position: 'top-start',
                                type: 'success',
                                title: 'شغل شما ثبت شد',
                                showConfirmButton: false,
                                timer: 2000
                            });
                            this.$router.push('/business/panel')
                        }
